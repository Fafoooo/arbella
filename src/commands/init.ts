...
export async function run(opts: InitOptions): Promise<void> {
  ...
  try {
    ...
    log.warn(
      "Config was saved, but the local clone failed. Fix access (auth/network) " +
        "then re-run `arbella init` or `arbella backup`.",
    );
  ...
  outro("arbella is configured. Run `arbella backup` any time to snapshot your setup.");
}
...
async function maybeRunFirstBackup(opts: InitOptions): Promise<void> {
  ...
  if (opts.yes) {
    ...
    if (isCancel(answer)) {
      ...
      log.step("Skipped the first backup. Run `arbella backup` when ready.");
      return;
    }
    go = answer;
  }
  ...
  try {
    ...
    if (typeof mod.run !== "function") {
      log.warn("Backup command unavailable; run `arbella backup` manually.");
      return;
    }
    ...
    log.warn(
      `First backup did not complete: ${errMessage(err)}. ` +
        "You can run `arbella backup` later.",
    );
  }
}
