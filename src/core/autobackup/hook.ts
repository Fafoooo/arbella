 ...
 * a session triggers `arbella backup --auto` in the background. The `--auto` path
...
/**
 * The command the hook runs. It launches `arbella backup --auto` detached so the
 ...
 *
 *   POSIX:  arbella backup --auto >/dev/null 2>&1 & # arbella-autobackup
 *   win32:  start /b "" arbella backup --auto >NUL 2>&1 :: arbella-autobackup
 *
...
export function hookCommand(): string {
  if (detectOS() === "win32") {
    ...
    return `start /b "" arbella backup --auto >NUL 2>&1 :: ${HOOK_TAG}`;
  }
  // POSIX: trailing `&` backgrounds it; the `#` comment carries the tag.
  return `arbella backup --auto >/dev/null 2>&1 & # ${HOOK_TAG}`;
}
