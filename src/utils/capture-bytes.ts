 ...
 * text path (sanitize + {{TOKEN}} folding) always runs. Genuinely binary content
 * (images, sqlite, real blobs) is reported as binary AND handed back as a lossy
 * UTF-8 view so callers can run a fail-safe secret scan before storing it raw.
 *
...
export function decodeForCapture(bytes: Buffer): DecodedBytes {
  ...
  if (nulTotal === 0) return { kind: "text", text: bytes.toString("utf8") };

  // No-BOM UTF-16: NULs strongly biased to one parity => interleaved ASCII text.
  // Require both a meaningful density and a lopsided ratio so genuine binaries
  // (NULs at both parities) are never misread as text.
  const pairs = Math.max(1, Math.floor(n / 2));
  const DENSITY = 0.3;
  const RATIO = 4;
  if (nulOdd >= pairs * DENSITY && nulOdd > nulEven * RATIO) {
    return { kind: "text", text: bytes.toString("utf16le") };
  }
  if (nulEven >= pairs * DENSITY && nulEven > nulOdd * RATIO) {
